...
if(isset($_GET['id'])){
	...
	if($_SESSION['ROLE']==2 && $_SESSION['USER_ID']!=$id){  
		die('Access denied');  // Ce constructeur de langage est équivalent à exit().
	}
	$res=mysqli_query($con,"select * from employee where id='$id'"); //user
...
if(isset($_POST['submit'])){
	...
	$password=mysqli_real_escape_string($con,$_POST['paswoord']);
	$department_id=mysqli_real_escape_string($con,$_POST['department_id']); // employe
	$address=mysqli_real_escape_string($con,$_POST['address']);
	$CIN=mysqli_real_escape_string($con,$_POST['CIN']);
	if($id>0){
		$sql="update employee set name='$name',email='$email',mobile='$mobile',paswoord='$password',department_id='$department_id',address='$address',CIN='$CIN' where id='$id'";
	}else{
		$sql="insert into employee(name,email,mobile,paswoord,department_id,address,CIN,role) values('$name','$email','$mobile','$password','$department_id','$address','$CIN','2')";
	}
                           ...
                           <form method="post">
							   <div class="form-group">
								...
								<div class="form-group">
									...
									<input type="password"  name="paswoord" placeholder="Enter employee password" class="form-control" required>
								</div>
								<div class="form-group">
									...
									<select name="department_id" required class="form-control">
										...
										while($row=mysqli_fetch_assoc($res)){
											if($department_id==$row['id']){
												echo "<option selected='selected' value=".$row['id'].">".$row['department']."</option>"; //select departement
											}else{
